...
// Returns a sibling category so the Category filter chip has multiple options to test
const relatedCategory = (category: string): string => {
  ...
  return 'Ottoman'; // generic fallback
};
...
export const createMockCandidates = (category: string, style: string): RawProductCandidate[] => {
  ...
  // Image seeds are fixed offsets so they stay stable across hot-reloads
  const img = (seed: number) =>
    `https://picsum.photos/400/500?random=${seed}`;

  return [
    // ── HIGH match scores (title contains both style + category) ─────────────
    {
    ...
    // ── MEDIUM match scores (title contains only category) ───────────────────
    {
    ...
    // ── LOW match scores (different/related category, no style in title) ─────
    {
